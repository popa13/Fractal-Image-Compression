...
namespace ImageTools.Compression.Fractal
{
    public class FractalCompression
    {
        ...
        public FractalCompressionResult Compress(CustomImage image)
        {
            ...
            int maxDxPos = image.Width - _options.DSquareSize + 1;
            int maxDyPos = image.Height - _options.DSquareSize + 1;
            ...
            Parallel.For(0, image.Width, dx =>
            {
                for (int dy = 0; dy < image.Height; dy++)
                {
                    ...
                    imgDataI[dx, dy] = rawImgData[dx, dy].IDownSampled;
                    imgDataQ[dx, dy] = rawImgData[dx, dy].QDownSampled;
                }
            });
                    ...
                    for (i = 0; i < transformationsCount; i++)
                    {
                        for (x = 0; x < dSize; x += drInverseRatio) //Increment of inverse ratio to get only parts of Di
                        {
                            for (y = 0; y < dSize; y += drInverseRatio)
                            {
                                ...
                                newX = Convert.ToByte(tempTransformMatrix[0, 0] * (x / drInverseRatio) + tempTransformMatrix[0, 1] * (y / drInverseRatio) + tempTranslateMatrix[0]);
                                newY = Convert.ToByte(tempTransformMatrix[1, 0] * (x / drInverseRatio) + tempTransformMatrix[1, 1] * (y / drInverseRatio) + tempTranslateMatrix[1]);

                                indexX = Convert.ToInt32(newX * drRatio);
                                indexY = Convert.ToInt32(newY * drRatio);
            ...
            for (rx = 0; rx < rXCount; rx++)
            {
                ...
                Parallel.For(0, rYCount, ry =>
                {
                    ...
                    transformations[ry * rXCount + rx, 0] = new FractalCompressionResult.FractalTransformation(bestRmsYIndexX, bestRmsYIndexY, Convert.ToByte(bestBrightnessY), Convert.ToByte(bestContrastY), Convert.ToByte(bestRmsYTransformationIndex));
                    transformations[ry * rXCount + rx, 1] = new FractalCompressionResult.FractalTransformation(bestRmsIIndexX, bestRmsIIndexY, Convert.ToByte(bestBrightnessI), Convert.ToByte(bestContrastI), Convert.ToByte(bestRmsITransformationIndex));
                    transformations[ry * rXCount + rx, 2] = new FractalCompressionResult.FractalTransformation(bestRmsQIndexX, bestRmsQIndexY, Convert.ToByte(bestBrightnessQ), Convert.ToByte(bestContrastQ), Convert.ToByte(bestRmsQTransformationIndex));
                });
            }
        ...
        public CustomImage Uncompress(FractalCompressionResult compressionResult, CustomImage decodingImage)
        {
            ...
            Parallel.For(0, imgSizeX, dx =>
            {
                ...
                {
                    imgData[dx, dy, 0] = rawImgData[dx, dy].Y;
                    imgData[dx, dy, 1] = rawImgData[dx, dy].IDownSampled;
                    imgData[dx, dy, 2] = rawImgData[dx, dy].QDownSampled;
                }
            });
                ...
                for (int dy = 0; dy < maxDyPos; dy++)
                {
                    ...
                    for (x = 0; x < dSize; x += drInverseRatioAdapted) //Increment of inverse ratio to get only parts of Di
                    {
                        for (y = 0; y < dSize; y += drInverseRatioAdapted)
                        {
                            ...
                            indexX = Convert.ToInt32(y * drRatio);
                            indexY = Convert.ToInt32(x * drRatio);
            ...
            {
                int ry, indexX, indexY;
                for (int rx = 0; rx < rXCount; rx++)
                {
                    for (ry = 0; ry < rYCount; ry++)
                    {
                        for (indexX = 0; indexX < rSize; indexX++)
                        {
                            for (indexY = 0; indexY < rSize; indexY++)
                            {
                                pixels[rx * rSize + indexX, ry * rSize + indexY] = Pixel.FromYIQDownSampled(imgData[rx * rSize + indexX, ry * rSize + indexY, 0], imgData[rx * rSize + indexX, ry * rSize + indexY, 1], imgData[rx * rSize + indexX, ry * rSize + indexY, 2]);
                            }
                        }
                    }
                }
